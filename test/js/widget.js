			...
			if(start_canvas==true ){
				...
				for(var b=0; b<=line_num;b++){
					...
					if(ctx.isPointInPath(canvasX, canvasY)) {
						...
						widgetDraw(line_uid[line_num],line_num);
						//parent.Chart.Draw();
...
//		console.log("drawLineH Start:"+LT+":Hcolor:"+Hcolor);
	...
	var widgetNum;
	function widgetDraw(xy,wnum){
		widgetCheck = true;
		widgetNum   = wnum;
		$('#canvasDiv2').empty();
		console.log("추세선 그리기===>"+xy);
		var wXYd = xy.split("|");
		wXYd[0] = eval(wXYd[0]);
		wXYd[1] = eval(wXYd[1]);
		wXYd[2] = eval(wXYd[2]);
		...
		wXYd[7] =      wXYd[7];
		wXYd[8] =      wXYd[8];
		this.LW=0,this.RW=0,this.TH=0,this.BH=0;

		//CPriceSubGraph.prototype.Draw
		mainContext.lineWidth = 1;
		mainContext.strokeStyle = wXYd[8];
		mainContext.fillStyle   = wXYd[8];
		...
		switch (wXYd[6])
		{
			...
			case 'b':
			case 'c':
				mainContext.moveTo(wXYd[1]+LW, wXYd[2]);
				mainContext.lineTo(wXYd[3]+LW, wXYd[4]);
				mainContext.closePath();
				break;
			case 'd':
				mainContext.moveTo(0+LW,wXYd[4]);
				mainContext.lineTo(eval(wXYd[1])+LW,wXYd[4]);
				mainContext.moveTo(eval(wXYd[3])+LW,0);
				mainContext.lineTo(eval(wXYd[3])+LW,wXYd[2]);
				mainContext.closePath();
				break;
			case 'e':
				mainContext.rect(wXYd[1]+LW, wXYd[2],wXYd[3], wXYd[4]);
				mainContext.closePath();
				break;
			case 'f':
				var hrx = Math.abs(eval(wXYd[3])-eval(wXYd[1]));
				var hry = Math.abs(eval(wXYd[4])-eval(wXYd[2]));		   
				mainContext.arc(eval(wXYd[1])+LW + hrx/2, eval(wXYd[2]) + hry/2, hrx/2, 0, 2*Math.PI);  //원 그리기(360도 호 그리기)
				mainContext.closePath();
				break;
			case 'g':
				break;
			case 'h':
				break;
			case 'i':
				mainContext.moveTo(eval(wXYd[1])+LW,   eval(wXYd[2])); //패스 시작점 지정  
				mainContext.lineTo(eval(wXYd[1])+LW-5, eval(wXYd[2])+10); //패스 이동점 지정
				mainContext.lineTo(eval(wXYd[1])+LW+5, eval(wXYd[2])+10);  
				mainContext.lineTo(eval(wXYd[1])+LW,   eval(wXYd[2])); 
				mainContext.fillStyle = "#0000ff";
				mainContext.strokeStyle = "#0000ff";
				mainContext.fill();   //색 채우기
				mainContext.closePath();
				break;
			case 'j':
				mainContext.moveTo(eval(wXYd[1])+LW,   eval(wXYd[2])); //패스 시작점 지정  
			...
			case 'l':
				...
				mainContext.font = '14px sans-serif';
				//console.log(txt.fontsize(font));
				mainContext.fillText(txt,eval(wXYd[1])-4, eval(wXYd[2])-4);
				mainContext.closePath();
	...
	function widgetEdit(wx,wy){
		if(widgetCheck== true){
			
			var weXYd = line_uid[widgetNum].split("|");
			weXYd[0] = eval(weXYd[0]);
			weXYd[1] = eval(weXYd[1]);
			weXYd[2] = eval(weXYd[2]);
			weXYd[3] = eval(weXYd[3]);
			weXYd[4] = eval(weXYd[4]);
			weXYd[5] = eval(weXYd[5]);
			weXYd[6] =      weXYd[6];
			weXYd[7] =      weXYd[7];
			weXYd[8] =      weXYd[8];
			...
			console.log("widget:"+line_uid[widgetNum]+"==>"+this.LW+":"+weXYd[1]+":"+weXYd[2]+":"+weXYd[3]+":"+weXYd[4]);
			mainContext.putImageData(imageChart, 0, 0);
		}
	}
